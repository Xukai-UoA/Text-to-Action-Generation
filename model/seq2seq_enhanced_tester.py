"""
PyTorch version of seq2seq_enhanced_tester.py
用于测试训练好的增强seq2seq模型（支持Bahdanau Attention、残差连接、LayerNorm等改进）
"""
...
class Seq2SeqEnhancedTester:
    """
    增强Seq2Seq测试器
    用于加载训练好的增强模型并生成动作序列
    支持：Bahdanau Attention、多层LSTM、残差连接、Layer Normalization
    """
    ...
    def _load_model(self):
        ...
        print(f"  - Attention Size: {self.attention_size}")
    ...
    def test(self, test_script, test_script_len):
        ...
        with torch.no_grad():  # 测试时不需要梯度
            ...
            # 2. 从文本生成动作（测试时不需要ground_truth，teacher_forcing_ratio=0）
            action_gen_out, action_enc_out = self.model.char2action(
                char_enc_out, curr_init_input, curr_random, self.num_data,
                ground_truth_actions=None,  # 测试时不使用ground truth
                teacher_forcing_ratio=0.0    # 测试时完全使用autoregressive模式
            )
...
if __name__ == "__main__":
    ...
    save_action_to_file(generated_action,
                        os.path.join(output_dir, 'generated_action_enhanced.npy'))

    # 可视化
    print("\n可视化动作序列...")
    visualize_action_sequence(
        generated_action,
        title=f"Enhanced Generated Action: '{test_sentence}'",
        save_path=os.path.join(output_dir, 'action_visualization_enhanced.png')
    )
